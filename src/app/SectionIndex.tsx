...
export function SectionIndex({ section }: { section: Section }) {
  ...
  const groups = SECTION_GROUPS[section]
    .map((g) => ({ key: g.key, articles: getArticles({ section, lang, group: g.key }) }))
    .filter((g) => g.articles.length > 0)

  return (
    <div className="mx-auto max-w-3xl">
      <h1 className="text-3xl font-black tracking-tight sm:text-4xl">{t('title')}</h1>
      <p className="mt-3 text-base text-on-surface-variant sm:text-lg">{t('subtitle')}</p>

      <div className="mt-10 space-y-10">
        {groups.map((group) => (
          <section key={group.key}>
            <h2 className="border-b border-line pb-2 text-xl font-black">
              {t(`groups.${group.key}`)}
            </h2>
